...
/**
 * 来至dubbo框架，主要工作是改进 jdk 的Class.forName方法,支持原生类型加载和数组类型加载
 * 
 * @author
 *
 */
public class ClassHelper {
	...
	public static ClassLoader getClassLoader(Class<?> cls) {
		/**
		 * 逻辑：优先使用当前线程的类加载器，获取不到则使用加载该类的类加载器
		 */
	...
	public static Class<?> forName(String name, ClassLoader classLoader) throws ClassNotFoundException, LinkageError {
		...
		Class<?> clazz = resolvePrimitiveClassName(name);//原生类加载逻辑
		if (clazz != null) {
			return clazz;
		}
	...
	static {
		//将jdk 原生类和包装类对应关系   全部放入primitiveWrapperTypeMap
		...
		//将原生类及其对应的数组Class对象全部放入primitiveTypeNameMap
